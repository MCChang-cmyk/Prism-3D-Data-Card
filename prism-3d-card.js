...
class Prism3DCardEditor extends LitElement {
  ...
  _labelFor(name) {
    const labels = {
      title: "卡片標題", data_mode: "數據計算模式", decimal_places: "顯示小數點位數", // 新增標籤
      color: "圖表主色", mode: "顯示模式", chart_radius: "圖表縮放比例", 
    ...
    return html`
      ...
      <ha-form
        ...
        .schema=${[
          ...
          { name: "decimal_places", selector: { number: { min: 0, max: 5, step: 1, mode: "slider" } } }, // 新增小數點設定
...
class Prism3DCard extends HTMLElement {
  ...
  static getStubConfig() { return { mode: "3d", data_mode: "absolute", decimal_places: 1, color: "#E13460", rotation: 0, tilt: 0.4, entities: [], title: "數據稜鏡" }; }
  ...
  _updateData() {
    ...
    const dataValues = entities.map(ent => {
      ...
      // 優先序：實體設定的精度 > 使用者全域設定的精度
      const precision = stateObj?.attributes?.display_precision !== undefined 
        ? stateObj.attributes.display_precision 
        : globalDecimals;
    ...
    const indicators = entities.map(ent => {
      ...
      return { 
        name: (ent.name || stateObj?.attributes?.friendly_name || ent.entity.split('.')[1]).toUpperCase(), 
        max: ent.max || 100,
        unit: unit // 存入單位供 Tooltip 使用
      };
    });
      ...
      option = {
        ...
        tooltip: { show: !this._isDragging, trigger: 'item', enterable: false, confine: true, appendToBody: false, transitionDuration: 0, position: (pos) => [pos[0] + 20, pos[1] + 20], extraCssText: 'pointer-events: none !important; z-index: 9999; border:none !important; box-shadow:none !important;', backgroundColor: 'rgba(0, 0, 0, 0.85)', borderColor: mainColor, borderWidth: 1, textStyle: { color: '#fff', fontSize: 12 },
          formatter: (p) => { 
            const i = p.dataIndex; 
            if (i < 0) return '';
            return `<div style="padding: 5px;"><span style="color:#94a3b8; margin-right:15px;">${indicators[i].name}</span><b style="color:${mainColor}">${dataValues[i]} ${indicators[i].unit}</b></div>`; 
          }
        },
