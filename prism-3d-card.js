...
class Prism3DCardEditor extends LitElement {
  ...
  _labelFor(name) {
    const labels = {
      ...
      chart_radius: "圖表縮放比例", entities: "選擇實體 (Entities)", rotation: "旋轉角度", 
      drag_direction: "拖曳旋轉方向", tilt: "傾斜視角 (俯視度)", line_width: "稜線寬度 (0為不繪製)", 
      area_opacity: "區域總透明度", text_size: "文字字體大小", text_color: "文字顯示顏色", 
      text_stroke_width: "文字外框粗細", text_stroke_color: "文字外框顏色", 
      opacity_variation: "3D 明暗差異值", grid_color: "網格顏色", 
      grid_line_opacity: "網格線透明度", grid_opacity_1: "背景斑馬紋 - 淺色層", grid_opacity_2: "背景斑馬紋 - 深色層"
    };
  ...
  // --- 核心修正：解決 YAML 格式錯誤物件嵌套問題 ---
  _addEntity(ev) {
    const value = ev.detail.value;
    // 修正：從物件中提取字串 ID，而不是直接存入物件
...
class Prism3DCard extends HTMLElement {
  ...
  _initChart() {
    ...
    const onMove = (e) => { if (!this._isDragging) return; const x = e.pageX || (e.touches && e.touches[0].pageX); const deltaX = x - this._startX; const multiplier = (this.config.drag_direction === 'reverse' ? -0.5 : 0.5); this._dragRotation = Math.max(-90, Math.min(90, deltaX * multiplier)); this._updateData(); };
  ...
  _updateData() {
    ...
    const textSize = parseFloat(this.config.text_size) || 11;
    const textColor = this.config.text_color || '#94a3b8';
    const textStrokeWidth = this.config.text_stroke_width !== undefined ? parseFloat(this.config.text_stroke_width) : 2;
    ...
    if (is3D) {
      ...
      option = {
        ...
        series: [{ type: 'custom', renderItem: (params, api) => {
          ...
          return { type: 'text', z: 10, style: { text: indicators[i].name, x: pCurr.x, y: pCurr.y - 15, fill: isHovered ? '#fff' : textColor, font: `${isHovered ? 'bold ' : ''}${textSize}px sans-serif`, textAlign: 'center', textVerticalAlign: 'bottom', stroke: textStrokeColor, lineWidth: textStrokeWidth } };
      ...
      option = {
        ...
        radar: { indicator: indicators.map(ind => ({ ...ind, max: 1 })), startAngle: 90 + (parseFloat(this.config.rotation) || 0), shape: 'polygon', radius: `${parseFloat(this.config.chart_radius)||65}%`, center: ['50%', this.config.title ? '60%' : '50%'], axisName: { fontSize: textSize, color: textColor, stroke: textStrokeColor, lineWidth: textStrokeWidth }, splitLine: { lineStyle: { color: this._hexToRgba(gridColor, gridLineOp) } }, splitArea: { show: true, areaStyle: { color: [this._hexToRgba(gridColor, parseFloat(this.config.grid_opacity_2)||0.05), this._hexToRgba(gridColor, parseFloat(this.config.grid_opacity_1)||0.02)].reverse() } } },
