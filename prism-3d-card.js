  ...
  _updateData() {
    ...
    if (is3D) {
      ...
      option = {
        ...
        radar: {
          indicator: indicators, startAngle: 90 + (parseFloat(this.config.rotation) || 0), shape: 'polygon', radius: `${chartRadiusVal}%`, center: ['50%', this.config.title ? '60%' : '50%'],
          ...
          splitLine: { lineStyle: { color: this._hexToRgba(this.config.grid_color || '#fff', gridLineOp) } },
          splitArea: { show: true, areaStyle: { color: [this._hexToRgba(this.config.grid_color || '#fff', gOp2), this._hexToRgba(this.config.grid_color || '#fff', gOp1)].reverse() } }
        },
        series: [{ type: 'radar', data: [{ value: dataValues, symbol: 'none', lineStyle: { color: mainColor, width: lineWidth }, areaStyle: { color: this._hexToRgba(mainColor, areaOpacity) } }] }]
      };
    }
